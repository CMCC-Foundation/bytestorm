...
class TCDetectionDatasetWriter():
    def __init__(
            self, 
            src_dir: str, 
            georef: pd.DataFrame, 
    ...
    def process_year(self,
        ...
        tc_S = np.empty(shape=(0, 1, 1), dtype=object)
        ...
        for _, filepath in zip(tqdm(filepaths), filepaths):
            ...
            ds = self._create_tc_maps(ds=ds)
            # get dataset iso time
            iso_time = self._get_iso_time(ds)
            ...
            tc_ids, no_tc_ids, _, nr_ids, rn_ids = self._get_patch_ids(ds, georef)
            # fill patch dataset with tc info
            patch_ds = self._fill_patch_ds_with_tc_info(patch_ds, tc_ids, georef)
            ...
            S_data = np.transpose(patch_ds.isel(time=0)[['sid_cyclone']].to_array().data, axes=(1,2,0,3))
            # update datasets
            try:
                tc_X, tc_Y, tc_S = self._update_dataset(tc_X, tc_Y, X_data, Y_data, tc_ids, tc_S, S_data)
            except:
                logging.info(f'tc_S {tc_S}')
                logging.info(f'S_data {S_data}')
                tc_X, tc_Y = self._update_dataset(tc_X, tc_Y, X_data, Y_data, tc_ids)
            if is_test == True:
                no_tc_X, no_tc_Y = self._update_dataset(no_tc_X, no_tc_Y, X_data, Y_data, no_tc_ids)
            if is_test == False:
                nr_X, nr_Y = self._update_dataset(nr_X, nr_Y, X_data, Y_data, nr_ids)
            if is_test == False:
                rn_X, rn_Y = self._update_dataset(rn_X, rn_Y, X_data, Y_data, rn_ids)
        # store datasets to disk
        self._create_zarr(tc_X, tc_Y, tc_dst, tc_S)
        self._create_zarr(no_tc_X, no_tc_Y, no_tc_dst) if is_test == True else None
        self._create_zarr(nr_X, nr_Y, nr_dst) if is_test == False else None
        self._create_zarr(rn_X, rn_Y, rn_dst) if is_test == False else None

    def _create_zarr(self, X, Y, dst, S = None):
        if X.shape[0] == 0:
            return
        ...
        if S is not None:
            S = np.array(list(S[:,0,0]))
            ds['track_id'] = (('pid','sid'), S)
    ...
    def process_years_MPI(self, 
        ...
        for i,year in enumerate(years):
            if i % self.size != self.rank:
                continue
            self.process_year(dst_dir=dst_dir, year=year, months=months, is_test=is_test)
        return self
    ...
    def merge_zarr_files(self, 
        ...
        zarr_dst = os.path.join(dst_dir, pathname.split('-')[-1])
        # create zarr file
        self._create_zarr(X, Y, zarr_dst)
        # remove redundant zarr files
        for file in zarr_files:
            shutil.rmtree(file)

    def _create_tc_maps(self, 
            ds:xr.Dataset
        ) -> xr.Dataset:
        ds = get_tropical_cyclone_positions(ds=ds, georef=self.georef, sigma=self.sigma, coords=('lat','lon'))
        return ds
    ...
    def _fill_patch_ds_with_tc_info(self, 
            patch_ds: xr.Dataset, 
            tc_ids: List, 
            georef: pd.DataFrame
        ):
        ...
        sid_cyclone_data = np.full(shape=(patch_ds.time.data.shape[0], patch_ds.rows.data.shape[0], patch_ds.cols.data.shape[0], 1), fill_value=np.nan, dtype=object)
        # for each rowcol coordinate
        for tc_id, pt_rowcol, rcy_coo, gl_rowcol, cy_coo, sid in zip(tc_ids, pcy_rowcol, rcy_coords, gcy_rowcol, cy_coords, cy_sids):
            i,j = tc_id
        ...
        patch_ds = patch_ds.assign({
            ...
            'sid_cyclone': (('time', 'rows', 'cols', 'sid'), sid_cyclone_data),
        })
        return patch_ds



class TCDetectionMPIDatasetWriter(TCDetectionDatasetWriter):
    def __init__(self, 
            src_dir: str, 
            georef: pd.DataFrame, 
            patch_vars: List[str], 
    ...
    def process(
            ...
            months, 
            is_test: bool = False):
        self.process_years_MPI(dst_dir=dst_dir, years=years, months=months, is_test=is_test)
